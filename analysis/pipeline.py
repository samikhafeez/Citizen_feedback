...
def main():
    ...
    if args.db:
        ...
        if embedder.backend == "hash":
            raise RuntimeError(
                f"Cannot upload to Supabase: the local e5 model failed to load, so the "
                f"pipeline fell back to the hash embedder ({embedder.dim}-dim). The database "
                f"expects 384-dim vectors (intfloat/multilingual-e5-small).\n\n"
                f"Fix: install sentence-transformers in your venv:\n"
