    ...
    <div>
      ...
      {res && (
        <>
          ...
          <p className="small" style={{ marginTop: 10, padding: 10, background: res.retrievalMode === "semantic" ? "#ebfbf0" : "#fbeede", borderRadius: 8 }}>
            {res.retrievalMode === "semantic" ? (
              <>
                ✅ This query used <strong>semantic search</strong> (local e5 embeddings + Supabase pgvector)
                over {res.totalFreeText} matched answer(s).
              </>
            ) : (
              <>
                ⚠ A summary is evidence only if it cites its sources. This preview uses <strong>lexical (keyword)</strong>{" "}
