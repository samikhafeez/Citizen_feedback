...
export async function GET(req: Request) {
  ...
  if (view === "rag") {
    ...
    console.log("[RAG] DATA_BACKEND =", JSON.stringify(process.env.DATA_BACKEND), "| cwd =", process.cwd());
    if (process.env.DATA_BACKEND === "supabase") {
      ...
      console.log("[RAG] pythonExe =", pythonExe, "| exists =", fs.existsSync(pythonExe));
      console.log("[RAG] scriptPath =", scriptPath, "| exists =", fs.existsSync(scriptPath));
      ...
      if (fs.existsSync(pythonExe) && fs.existsSync(scriptPath)) {
        try {
          ...
          return NextResponse.json({ ...result, retrievalMode: "semantic" });
        } catch (e) {
      ...
      console.log("[RAG] Skipping semantic search — DATA_BACKEND is not 'supabase'");
    }

    // Default local lexical search fallback
    const result = await ragQuery(q, isNaN(k) ? 6 : k);
    return NextResponse.json({ ...result, retrievalMode: "lexical" });
  }
