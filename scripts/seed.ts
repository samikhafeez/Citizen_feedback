...
async function main() {
  ...
  // 2. clear existing options for this version (options can be safely re-inserted)
  const qIds = SURVEY.questions.map((q) => q.id);
  const { error: delOptErr } = await db.from("question_options").delete().in("question_id", qIds);
  if (delOptErr) throw delOptErr;

  // 3. questions (upsert to avoid foreign key violations on active responses)
  ...
  {
    const { error } = await db.from("questions").upsert(questionRows);
    if (error) throw error;
  }
